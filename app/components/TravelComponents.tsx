...
import { Train, Info, Utensils, Navigation2, MapPin, X, ExternalLink, Cloud, Sun, CloudRain, Save } from 'lucide-react';
...
export const SpotCard = ({ spot, onClick, colors }: any) => (
  <div className="bg-white rounded-[3rem] p-8 shadow-sm border border-pink-50 transition-all active:scale-[0.98]" onClick={() => onClick(spot)}>
    ...
    <div className="flex items-center gap-2 mt-4 text-[9px] opacity-40 uppercase tracking-[0.1em]">
      ...
      {/* 顯示帶逗號的金額 */}
      <span>{formatMoney(spot.cost, spot.currency || 'JPY')}</span>
    </div>
  </div>
);
...
// 支援金額修改的 DetailModal
export const DetailModal = ({ spot, onClose, onNav, onUpdateCost, colors }: any) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editCost, setEditCost] = useState(spot.cost);

  const handleSave = () => {
    onUpdateCost(spot.id, Number(editCost));
    setIsEditing(false);
  };
      ...
      <div className="bg-white w-full max-w-md rounded-t-[4.5rem] p-12 shadow-2xl animate-in slide-in-from-bottom duration-600 overflow-y-auto max-h-[94vh]">
        ...
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            {/* 可編輯的預算區塊 */}
            <div className="bg-slate-50/80 p-6 rounded-[2.5rem] relative">
              <h4 className="text-[9px] uppercase tracking-[0.2em] mb-4 flex items-center gap-2" style={{ color: colors.sub }}>
                <div className="w-1.5 h-1.5 bg-slate-400 rounded-full"></div>預算 (可修改)
              </h4>
              {isEditing ? (
                <div className="flex items-center gap-2">
                  <input 
                    ...
                    className="w-full bg-white p-2 rounded-xl text-xs outline-none border border-pink-200"
                  />
                  <button onClick={handleSave} className="p-2 bg-slate-800 text-white rounded-full">
                    <Save size={12}/>
                  </button>
                </div>
              ) : (
                <p className="text-[12px] font-mono font-bold text-slate-600 cursor-pointer hover:text-pink-400 transition-colors" onClick={() => setIsEditing(true)}>
                  {formatMoney(spot.cost, spot.currency || 'JPY')}
                </p>
              )}
            </div>
          </div>
